...
function switchTheory(manualSwitch = false) {
	...
	for (let i = 0; i < Math.min(8, game.researchUpgrades[7].level); i++) {
        ...
        if(i == 1) {
            value = value * 10.2; //T2 overpush coefficient.
        } else if(i == 3) {
            value = value * 1.6; //T4 overpush coefficient. 
        } else if(i == 5) {
            value = value * 2.8; //T6 overpush coefficient.
        } else if(i == 4) {
            value = value * 1.2; //T5 overpush coefficient. 
        }
		if (value > max) {
			iMax = i;
			max = value;
